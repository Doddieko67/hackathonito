...
Completa OAuth de Google y exige onboarding mínimo si faltan datos.
Nota: el inicio del flujo OAuth lo maneja Supabase directamente — no requiere endpoint propio.
El callback de Supabase aterriza en app/auth/callback/route.ts.
...
## 3. Chat con IA (Gemini function calling)

### POST /api/chat

Conversación con Gemini. El modelo decide si llamar funciones (buscar negocios, agregar evento,
leer itinerario) o responder con texto. El historial viaja en cada request desde el cliente.

Auth: Turista

Body:
{
  "mensaje": "¿dónde puedo comer tacos cerca?",
  "historial": [
    { "role": "user",  "text": "Hola" },
...
Reglas:
- historial opcional — si viene vacío, Gemini inicia conversación nueva.
- lat/lng opcionales — si vienen, se usan para buscar negocios cercanos.
- Gemini puede encadenar múltiples function calls antes de responder.

Response 200:
{
  "ok": true,
  "data": {
    "respuesta": "Encontré 2 taquerías cerca: Tacos El Güero a 120m...",
    "eventoAgregado": {
      "negocio_id": "neg-001",
...
Nota: eventoAgregado solo aparece cuando Gemini llamó agregar_evento en esa respuesta.

---

## 4. Cuestionario, recomendaciones e itinerarios
...
### PATCH /api/itinerary/{itineraryId}
...
### POST /api/itinerary/{itineraryId}/save
...
### POST /api/itinerary/{itineraryId}/pdf
...
### GET /api/itinerary/{itineraryId}/route
...
### POST /api/itinerary/{itineraryId}/route/rebuild
...
## 5. Solicitud de registro de negocio (EncargadoDelNegocio)
...
## 6. Revisión administrativa de solicitudes
...
## 7. Perfil de negocio (tras aprobación)
...
## 8. Registro de visitas para algoritmo de equidad
...
Body:
{
  "title": "Error 500 en /api/itinerary",
  "description": "...",
  "severity": "high"
}
...
|---|---:|---:|---:|---:|
| POST /api/recommend | Si | No | No | No |
| POST /api/itinerary | Si | No | No | No |
| PATCH /api/itinerary/{id} | Si (dueno) | No | No | No |
| GET /api/itinerary/{id}/route | Si (dueno) | No | No | No |
| POST /api/itinerary/{id}/route/rebuild | Si (dueno) | No | No | No |
...
## 11. Pendientes menores antes de cerrar final
...
| 2026-04-06 | Fidel | v1.2 — paths en inglés, OAuth simplificado (Supabase callback), endpoint /api/chat documentado. |
